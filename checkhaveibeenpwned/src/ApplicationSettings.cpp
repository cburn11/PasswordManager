...
ApplicationSettings::ApplicationSettings(std::wstring&& AppKey) {
	
	HKEY	hApplicationKey;

	m_AppKey = std::move(AppKey);

	auto lRes = RegOpenKey(HKEY_CURRENT_USER, m_AppKey.c_str(), &hApplicationKey);
	if( lRes == ERROR_SUCCESS ) {
		...
		lRes = RegCreateKey(HKEY_CURRENT_USER, m_AppKey.c_str(), &hApplicationKey);
